                        ...
                        <DialogPanel
                            ...
                            <header class="py-3 px-4 flex justify-between items-center">
                                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                    {{ customer.id ? `Actualizeaza clientul: "${props.customer.first_name} ${props.customer.last_name}"` : 'Create new Customer' }}
                                </DialogTitle>
                                <button
                                    @click="closeModal()"
                          ...
                          text-white bg-orange-600 hover:bg-orange-700 focus:ring-orange-500">
                                    ...
                                    <button type="button"
                                            class="mt-3 w-full inline-flex ju  stify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
...
const billingCountry = computed(() => {
    return store.state.countries.find(c => c.code === customer.value.billingAddress.country_code) || {states: null};
});
...
const shippingCountry = computed(() => {
    return store.state.countries.find(c => c.code === customer.value.shippingAddress.country_code) || {states: null};
});
