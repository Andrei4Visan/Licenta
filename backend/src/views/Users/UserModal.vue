                        ...
                        >
                            <header class = "py-3 px-4 flex justify-between items-center">
                                <DialogTitle as = "h3" class = "text-lg leading-6 font-medium text-gray-900">
                                    {{ user.id ? `Actualizează produsul: "${props.user.name}" ` : 'Creează un produs nou' }}
                                </DialogTitle>
                                <button
                                    @click = "closeModal()"
                            ...
                            <form @submit.prevent = "onSubmit()">
                                <div class="bg-white px-4 pt-5 pb-4">
                                    <CustomInput class = "mb-2" v-model="user.name" label="Nume"/>
                                    <CustomInput class = "mb-2" v-model="user.email" label="Email"/>
                                    <CustomInput type = "password" class = "mb-2" v-model="user.password" label="Parola"/>
                                </div>
        ...
        store.dispatch('createUser', user.value)
            .then(response => {
                if (response.status === 201) {
                    store.dispatch('getUsers', {
                        url: '/users', // Provide the URL parameter here
                        search: '',
                        perPage: 10,
                        sort_field: '',
                        sort_direction: ''
